...
func TestNewKESAddress(t *testing.T) {
	tests := []struct {
		...
		{
			input: maddr.MustParse("urn:kes:momo:mpesa:254712345678"),
			expected: &maddr.KESMobileMoneyAddress{
				MoneyAddress: maddr.MustParse("urn:kes:momo:mpesa:254712345678"),
				Carrier:      "mpesa",
				Phone:        "254712345678",
			},
			expectErr: false,
		...
		{
			input:     maddr.MustParse("urn:kes:momo:mpesa"),
			expectErr: true,
		},
		{
			input:     maddr.MustParse("urn:btc:addr:1LMcKyPmwebfygoeZP8E9jAMS2BcgH3Yip"),
			expectErr: true,
		},
	}
